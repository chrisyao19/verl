...
class DAPORewardManager(NaiveRewardManager):
    def __init__(
        ...
        super().__init__(tokenizer, num_examine, compute_score, reward_fn_key, qps, max_concurrency, timeout)
    ...
    def compute_scores(self, reward_data: DataProto) -> list[int | float | dict]:
        ...
        if self.overlong_buffer_cfg.enable:
            ...
            scores = [({**score, "score": score["score"] + overlong_reward} if isinstance(score, dict) else score + overlong_reward)
                      for score, overlong_reward in zip(scores, overlong_rewards)]
