...
class BaseRewardManager(ABC):
    ...
    def preprocess_reward_data(self, data: DataProto) -> DataProto:
        ...
        prompt_strs = np.array([
            self.tokenizer.decode(prompt_ids[i][-valid_len.item() :], skip_special_tokens=True)
            for i, valid_len in enumerate(valid_prompt_length)
        ])
        solution_strs = np.array([
            self.tokenizer.decode(response_ids[i][: valid_len.item()], skip_special_tokens=True)
            for i, valid_len in enumerate(valid_response_lengths)
        ])
    ...
    def log_data(self, data: DataProto, raw_reward: list[int | float | dict]) -> None:
    ...
    def __call__(self, data: DataProto, return_dict=False):
        ...
        self.log_data(data, raw_rewards)
